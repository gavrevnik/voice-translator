...
Офлайн-перевод доступен только на arm64-устройствах с Android 9+ и только для пары русский/сербский в обоих направлениях. Для сербского можно выбрать латиницу (по умолчанию) или кириллицу. INT8-веса не входят в APK: приложение скачивает отдельный проверяемый пакет размером **45,0 MB** (**66,8 MB** после установки) и сохраняет его между обновлениями APK.
...
- `android/tools/` — воспроизводимая INT8-конвертация OPUS model pack и сборка JNI runtime.
