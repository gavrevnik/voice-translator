...
- Распознавание: **Auto** по умолчанию, а также ручные **Android/Samsung SpeechRecognizer**, **Groq Whisper Large V3**, локальный пакетный **Whisper Offline** и **Whisper Offline Live** (`large-v3-turbo-q4_0`, multilingual).
...
Настройки открываются отдельным прокручиваемым экраном. В порядке pipeline идут списки распознавания и перевода; Android TTS теперь фиксирован и не требует отдельного выбора. Ниже показываются статусы API-ключей и кнопка **Export logs**. Она открывает системное меню «Поделиться» для диагностического файла последнего речевого цикла, включая незавершившийся ошибкой цикл. В файл входят конфигурация движков, безопасные сведения об устройстве, этапы, тайминги и технические ошибки; для локального Whisper дополнительно записываются внутренние времена mel/sample/encode/decode/prompt, количество проходов и fallback-счётчики. Аудио, распознанный и переведённый текст, а также API-ключи не записываются. OpenAI-ключ добавляется или заменяется в отдельном защищённом диалоге. Системная инструкция перевода совпадает с веб-версией, содержит канонические имена и коды языков, а transcript отправляется отдельным user input.
...
Локальное распознавание использует multilingual Whisper Large V3 Turbo в квантизации `Q4_0`. Оно доступно для всех шести языков приложения и не зависит от языковых пакетов Samsung/Google. Нативный arm64 runtime входит в APK, а веса скачиваются отдельно после выбора одного из Whisper Offline-режимов в настройках:

- модель: `ggml-large-v3-turbo-q4_0.bin`;
- скачивание и размер после установки: `473 992 235` байт (474,0 MB);
- SHA-256: `2ceb779ca61ce46ed6e9aa43c4a0f8a89a37f683df3c0ba3119238fcd5852155`;
- локальный artifact: `model-packs/ggml-large-v3-turbo-q4_0.bin` (игнорируется Git).

Delivery manifest находится в `app/src/main/assets/whisper_models.json` и указывает на GitHub Release `offline-whisper-large-v3-turbo-q4_0-v1`. URL можно переопределить через `OFFLINE_WHISPER_MODEL_URL` в корневом `.env`. Квантизация официальной multilingual Large V3 Turbo-модели воспроизводится командой `tools/quantize_whisper_model.sh`; нативный runtime собран из закреплённого commit `whisper.cpp` и хранится в `app/libs/whisperlib-release.aar` без весов. При первом запуске обновлённой версии каталоги снятых с поддержки Base и Small удаляются автоматически.

В обоих локальных Whisper-режимах Turbo заранее загружается до начала записи и остаётся в памяти между фразами. Распознавание и Silero VAD используют шесть CPU-потоков. Перед финальным decode VAD обрезает только внешнюю тишину, сохраняя внутренние паузы и защитные поля 250 мс до и 350 мс после речи; при отсутствии уверенных границ или экономии менее 500 мс используется исходная запись. Silero VAD v6.2.0 весом 885 098 байт входит в APK. Live-проход использует шестисекундное скользящее окно, greedy decode и целевой интервал 1500 мс; при медленном декодировании следующий проход получает адаптивную паузу, чтобы не загружать CPU непрерывно. После Stop приложение дописывает 200 мс звука, нативно прерывает текущий partial decode и запускает отдельный финальный beam-search по подготовленной полной реплике.

`Whisper Offline` выполняет один decode после Stop. `Whisper Offline Live` каждые 1500 мс последовательно декодирует rolling-window последних 6 секунд через быстрый greedy/beam-1 проход без timestamps, передавая заранее выбранный код языка и небольшой текстовый prompt из уже собранного контекста. Одновременно выполняется не больше одного inference, поэтому очередь запросов не растёт. Второе нажатие оставляет микрофон активным ещё 200 мс, затем запускает отдельный финальный decode всей сохранённой речи с beam-3, отключёнными timestamps и температурными fallback-проходами и выбранным пользователем языком; только его результат передаётся в перевод. Это уменьшает наиболее дорогую часть задержки после Stop, сохраняя beam search для качества окончательного текста. Partial-текст отмечен в UI и показан менее контрастно.

Замеры пишутся с тегом `SayItTiming`: preloading, VAD и факт обрезки, исходная/декодируемая длительность аудио, latency первого partial, средний интервал обновлений, длительность partial/final decode, Stop-to-final, process CPU time, PSS и overrun относительно интервала 1500 мс. Экспорт последнего цикла также содержит нативную разбивку Whisper на mel/sample/encode/decode/prompt, число запусков каждой стадии и количество fallback-проходов. На подключённом телефоне их можно смотреть через `adb logcat -s SayItTiming`.
